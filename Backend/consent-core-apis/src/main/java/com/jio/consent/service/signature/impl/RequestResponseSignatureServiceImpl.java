package com.jio.consent.service.signature.impl;

import com.auth0.jwt.JWT;
import com.auth0.jwt.JWTVerifier;
import com.auth0.jwt.algorithms.Algorithm;
import com.jio.consent.constant.ErrorCodes;
import com.jio.consent.dto.RecipientType;
import com.jio.consent.entity.JwkKey;
import com.jio.consent.exception.ConsentException;
import com.jio.consent.repository.signature.BusinessKeyRepository;
...
import com.jio.consent.utils.SignatureJsonSerializer;
...
public class RequestResponseSignatureServiceImpl implements RequestResponseSignatureService {
    ...
    @Override
    public boolean verifyRequest(Object payload, Map<String, String> headers) throws ConsentException {
        if (!verifyRequest) {
        ...
        try {
            ...
            String encodedCertificate = fetchCertificate(tenantId, businessId, dataProcessorId, requestorType);
            ...
            log.info("Request signature verification successful for tenant={}, requestorType={}", tenantId, requestorType);
            return true;

        } catch (ConsentException e) {
            // Re-throw consent exceptions
            throw e;
        } catch (Exception e) {
            log.error("Unexpected error during signature verification: {}", e.getMessage(), e);
            throw new ConsentException(ErrorCodes.JCMP3020);
        }
    }

    @Override
    public String signResponse(String tenantId, Object payload) throws ConsentException {
        if (!signResponse) {
        ...
        try {
            ...
            JwkKey jwk = jwkKeyRepository.findFirstByTenantIdAndKtyAndUse(tenantId, "RSA", "sig");
            if (jwk == null) {
                // Fallback: try finding by use only
                jwk = jwkKeyRepository.findFirstByTenantIdAndUse(tenantId, "sig");
            }
            if (jwk == null) {
                // Fallback: try finding by kid ordering (for keys with kid field)
                jwk = jwkKeyRepository.findFirstByTenantIdOrderByKidAsc(tenantId);
            }
            if (jwk == null) {
                log.error("No JWK key found in auth_key collection for tenant={}", tenantId);
                throw new ConsentException(ErrorCodes.JCMP3009);
            }

            log.debug("Using JWK key: kty={}, use={}, kid={} for tenant={}", jwk.getKty(), jwk.getUse(), jwk.getKid(), tenantId);

            if (jwk.getD() == null || jwk.getD().isBlank()) {
                log.error("JWK key missing private exponent (d parameter)");
                throw new ConsentException(ErrorCodes.JCMP3014);
            }
            ...
            log.error("Unexpected error during signature generation: {}", e.getMessage(), e);
            throw new ConsentException(ErrorCodes.JCMP3020);
        }
    }
     ...
     */
    private void validateVerificationHeaders(String tenantId, String requestorType, String signature) throws ConsentException {
        if (tenantId == null || tenantId.isBlank()) {
            throw new ConsentException(ErrorCodes.JCMP3011);
        }
        if (requestorType == null || requestorType.isBlank()) {
            throw new ConsentException(ErrorCodes.JCMP3011);
        }
        if (signature == null || signature.isBlank()) {
            throw new ConsentException(ErrorCodes.JCMP3012);
        }
    }
     ...
     */
    private RecipientType parseRecipientType(String requestorTypeStr) throws ConsentException {
        try {
            return RecipientType.valueOf(requestorTypeStr.toUpperCase());
        } catch (IllegalArgumentException e) {
            log.error("Invalid requestor type: {}", requestorTypeStr);
            throw new ConsentException(ErrorCodes.JCMP3013);
        }
    }
     ...
     */
    private String fetchCertificate(String tenantId, String businessId, String dataProcessorId, RecipientType requestorType) throws ConsentException {
        ...
        switch (requestorType) {
            case DATA_FIDUCIARY:
                ...
                if (encodedCertificate == null) {
                    throw new ConsentException(ErrorCodes.JCMP3010);
                }
                break;
            ...
            case DATA_PROCESSOR:
                ...
                encodedCertificate = businessKeyRepository.findCertificateByDataProcessorId(tenantId, dataProcessorId);
                if (encodedCertificate == null) {
                    throw new ConsentException(ErrorCodes.JCMP3010);
                }
                break;
     ...
     */
    private PublicKey extractPublicKeyFromCertificate(String encodedCertificate) throws ConsentException {
        try {
            // Decode certificate
            String certificate = decodeCertificate(encodedCertificate);
            if (certificate == null) {
                throw new ConsentException(ErrorCodes.JCMP3016);
            }
     ...
     */
    private String reconstructJwt(String detachedJwt, String payloadJson) throws ConsentException {
        try {
            ...
            if (parts.length != 2) {
                ...
                throw new ConsentException(ErrorCodes.JCMP3017);
            }
     ...
     */
    private void verifyJwtSignature(String jwt, PublicKey publicKey) throws ConsentException {
        try {
     ...
     */
    private RSAPrivateKey convertJwkToPrivateKey(JwkKey jwk) throws ConsentException {
        try {
            ...
            throw new ConsentException(ErrorCodes.JCMP3019);
        }
    }
     ...
     */
    private void validateJwkParameters(JwkKey jwk) throws ConsentException {
     ...
     */
    private BigInteger decodeBase64UrlToBigInteger(String base64Url, String parameterName) throws ConsentException {
        try {
            ...
            throw new ConsentException(ErrorCodes.JCMP3015);
        }
    }
     ...
     */
    private String convertToDetachedJwt(String fullJwt) throws ConsentException {
