...
> Central DPDP CMS documentation: see your internal wiki.
...
| **Context path & port** | `/grievance` on port `9007` (configurable) |
...
| --- | --- | --- | --- | --- |
| `SERVER_PORT` | HTTP listener port. Must align with service mesh and ingress. | none (`9007` in k8s config) | Yes | No |
...
```bash
curl -X POST http://localhost:9007/grievance/api/v1/grievances \
...
- **Kubernetes**: `deployment/Public_deployment.yaml` deploys the service into `dpdp-consent-public` namespace, exposes port `9007`, and mounts Kerberos artifacts plus log directories.
...
- **Product & engineering**: Jio.ConsentSupport@ril.com
- **Security disclosures**: Jio.ConsentSupport@ril.com (also see `SECURITY.md`)
