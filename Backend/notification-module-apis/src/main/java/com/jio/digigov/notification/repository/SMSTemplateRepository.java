...
public interface SMSTemplateRepository extends MongoRepository<SMSTemplate, String> {
     ...
     */
    List<SMSTemplate> findByBusinessIdAndScopeLevelAndType(
        String businessId, ScopeLevel scopeLevel, NotificationType type);
    
    /**
     * Find active templates by business ID
     */
    List<SMSTemplate> findByBusinessIdAndStatus(String businessId, TemplateStatus status);
    
    /**
     * Find templates by business ID and status
     */
    @Query("{'business_id': ?0, 'status': {'$in': ?1}}")
    List<SMSTemplate> findByBusinessIdAndStatusIn(String businessId, List<TemplateStatus> statuses);
