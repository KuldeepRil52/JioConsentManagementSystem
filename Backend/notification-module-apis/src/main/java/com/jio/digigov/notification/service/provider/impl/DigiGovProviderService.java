        ...
        while (true) {
            try {
                ...
                if (response != null && response.containsKey("status") &&
                    ...
                    String messageId = response.containsKey("messageId") ?
                            String.valueOf(response.get("messageId")) : null;

                    log.info("Successfully sent email via DigiGov: messageId={}", messageId);
                ...
                if (!hasRetried && isAuthenticationError(e)) {
                    ...
                    if (tokenService instanceof com.jio.digigov.notification.service.cache.CachedTokenService) {
                        ((com.jio.digigov.notification.service.cache.CachedTokenService) tokenService)
                                .evictToken(config.getBusinessId(), CredentialType.CLIENT);
                    }
