...
public class TokenServiceImpl implements TokenService {
    ...
    public TokenResponseDto generateToken(String tenantId, String businessId, CredentialType type) {
        ...
        // Validate DigiGov configuration
    ...
    private void validateDigiGovConfig(NotificationConfig config) {
        ...
        if (config.getProviderType() != ProviderType.DIGIGOV) {
            throw new IllegalArgumentException("Token service only supports DigiGov provider, found: " + config.getProviderType());
        }
        ...
        if (details.getClientId() == null || details.getClientId().trim().isEmpty()) {
            throw new IllegalArgumentException("Client ID cannot be empty");
        }
        if (details.getClientSecret() == null || details.getClientSecret().trim().isEmpty()) {
    ...
    private TokenResponseDto callTokenAPI(NotificationConfig config, CredentialType type) {
        ...
        // Get network type from configurationJson
        NetworkType networkType = config.getConfigurationJson().getNetworkType() != null ?
                NetworkType.valueOf(config.getConfigurationJson().getNetworkType()) : NetworkType.INTRANET;
        ...
        if (type == CredentialType.ADMIN) {
            ...
            headers.set("Authorization", createBasicAuthHeader(
                    config.getConfigurationJson().getClientId(),
                    config.getConfigurationJson().getClientSecret()));
        }
