...
/**
 * Service for handling missed Email notifications.
 *
 * This service provides functionality to retrieve and process missed Email notifications
 * with support for pagination, filtering, and multi-tenant operations.
 */
@Service
@Slf4j
public class MissedEmailNotificationService extends MissedNotificationBaseService {

    private final NotificationEmailRepository emailRepository;

    public MissedEmailNotificationService(NotificationEmailRepository emailRepository,
    ...
    /**
     * Retrieves missed Email notifications with pagination and filtering.
     *
     ...
     * @return PagedResponseDto containing missed Email notifications
     */
    public PagedResponseDto<NotificationEmail> getMissedEmailNotifications(
        ...
        log.debug("Retrieving missed Email notifications for tenant: {}, businessId: {}, page: {}, size: {}",
        ...
        return executeNotificationQuery(
                ...
                page,
                size,
                "Missed Email notifications retrieved successfully",
                "Failed to retrieve missed Email notifications"
        );
    }

    /**
     * Retrieves missed Email notifications by specific status.
     *
     * @param tenantId Tenant identifier
     * @param businessId Business identifier for filtering
     ...
     * @return PagedResponseDto containing missed Email notifications
     */
    public PagedResponseDto<NotificationEmail> getMissedEmailNotificationsByStatus(
            String tenantId, String businessId, String status, LocalDateTime fromDate, LocalDateTime toDate,
            int page, int size) {

        log.debug("Retrieving missed Email notifications by status: {} for tenant: {}", status, tenantId);
        ...
        return executeNotificationQuery(
                ...
                page,
                size,
                "Missed Email notifications by status retrieved successfully",
                "Failed to retrieve missed Email notifications by status"
        );
    }
     ...
     */
    private PagedResponseDto<NotificationEmail> executeNotificationQuery(
        ...
        try {
            ...
            long totalElements = mongoTemplate.count(query, NotificationEmail.class);
            List<NotificationEmail> notifications = executeQuery(query, mongoTemplate, page, size);

            log.debug("Found {} missed Email notifications for tenant: {}", notifications.size(), tenantId);

            return buildPagedResponse(notifications, page, size, totalElements, successMessage);

        } catch (Exception e) {
            log.error("Error retrieving missed Email notifications for tenant {}: {}", tenantId, e.getMessage());
     ...
     */
    private List<NotificationEmail> executeQuery(Query query, MongoTemplate mongoTemplate, int page, int size) {
     ...
     */
    private PagedResponseDto<NotificationEmail> buildPagedResponse(
            List<NotificationEmail> notifications, int page, int size, long totalElements, String message) {

        PagedResponseDto.PaginationInfo paginationInfo = calculatePaginationInfo(page, size, totalElements);

        return PagedResponseDto.<NotificationEmail>builder()
                .data(notifications)
