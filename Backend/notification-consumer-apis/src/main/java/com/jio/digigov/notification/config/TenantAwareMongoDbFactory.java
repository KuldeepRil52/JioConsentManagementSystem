    ...
    public MongoDatabase getMongoDatabase() {
        try {
            ...
            if ("SYSTEM".equalsIgnoreCase(tenantId)) {
                log.debug("TenantAwareMongoDbFactory: Retrieved tenantId: SYSTEM, Routing to shared DB: {}", sharedDatabase);
            ...
            log.warn("TenantAwareMongoDbFactory: No tenant context found, falling back to shared database: {}. " +
                    "This might indicate a missing tenantId in the request or an operation outside a tenant context.",
                    sharedDatabase);
