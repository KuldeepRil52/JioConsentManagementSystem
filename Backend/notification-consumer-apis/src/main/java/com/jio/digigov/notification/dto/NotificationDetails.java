...
public class NotificationDetails {
    ...
    // SMTP-specific configuration fields (used when providerType = SMTP)
    @Schema(description = "SMTP server hostname (e.g., smtp.gmail.com)")
    @JsonProperty("smtpHost")
    private String smtpHost;

    @Schema(description = "SMTP server port (e.g., 587 for TLS, 465 for SSL, 25 for plain)")
    @JsonProperty("smtpPort")
    private Integer smtpPort;

    @Schema(description = "SMTP username for authentication")
    @JsonProperty("smtpUsername")
    private String smtpUsername;

    @Schema(description = "SMTP credentials for authentication (encrypted)")
    @JsonProperty("smtpPassword")
    private String smtpPassword;

    @Schema(description = "Default 'from' email address for SMTP")
    @JsonProperty("smtpFromEmail")
    private String smtpFromEmail;

    @Schema(description = "Default 'from' name for SMTP emails")
    @JsonProperty("smtpFromName")
    private String smtpFromName;

    @Schema(description = "Enable TLS for SMTP connection")
    @JsonProperty("smtpUseTls")
    @Builder.Default
    private Boolean smtpUseTls = true;

    @Schema(description = "Enable SSL for SMTP connection")
    @JsonProperty("smtpUseSsl")
    @Builder.Default
    private Boolean smtpUseSsl = false;

    @Schema(description = "Enable SMTP authentication")
    @JsonProperty("smtpAuthEnabled")
    @Builder.Default
    private Boolean smtpAuthEnabled = true;

    @Schema(description = "SMTP connection timeout in milliseconds")
    @JsonProperty("smtpConnectionTimeout")
    @Builder.Default
    private Integer smtpConnectionTimeout = 5000;

    @Schema(description = "SMTP socket timeout in milliseconds")
