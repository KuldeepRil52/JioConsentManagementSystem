...
public class EventConfigurationRepositoryImpl implements EventConfigurationRepository {
    ...
    /**
     * Finds an event configuration by business ID and event type with 3-step fallback.
     *
     * This method implements a 3-step fallback mechanism:
     * Step 1: Search by businessId + eventType
     * Step 2: If not found, search by scopeLevel=TENANT + eventType
     * Step 3: If not found, search by tenantId (in businessId field) + eventType
     *
     * @param businessId The unique identifier for the business unit
     * @param tenantId The tenant identifier (used for Step 3 fallback)
     ...
     */
    @Override
    public Optional<EventConfiguration> findByBusinessIdAndEventType(String businessId, String tenantId, String eventType, MongoTemplate mongoTemplate) {
        log.debug("Finding event configuration by businessId: {}, tenantId: {} and eventType: {}", businessId, tenantId, eventType);

        // Step 1: Try to find by businessId + eventType
        Query step1Query = new Query(Criteria.where("businessId").is(businessId).and("eventType").is(eventType));
        EventConfiguration result = mongoTemplate.findOne(step1Query, EventConfiguration.class);

        if (result != null) {
            log.debug("Event configuration found (Step 1: businessId match): businessId={}, eventType={}", businessId, eventType);
            return Optional.of(result);
        }

        // Step 2: If not found, try to find by scopeLevel=TENANT + eventType
        log.debug("Event configuration not found for businessId: {}, trying scopeLevel=TENANT fallback (Step 2)", businessId);
        Query step2Query = new Query(Criteria.where("scopeLevel").is(ScopeLevel.TENANT).and("eventType").is(eventType));
        result = mongoTemplate.findOne(step2Query, EventConfiguration.class);

        if (result != null) {
            log.info("Event configuration found using TENANT-level fallback (Step 2: scopeLevel=TENANT match): eventType={}", eventType);
            return Optional.of(result);
        }

        // Step 3: If still not found, try to find by tenantId (in businessId field) + eventType
        log.debug("Event configuration not found for scopeLevel=TENANT, trying tenantId as businessId fallback (Step 3)");
        Query step3Query = new Query(Criteria.where("businessId").is(tenantId).and("eventType").is(eventType));
        result = mongoTemplate.findOne(step3Query, EventConfiguration.class);

        if (result != null) {
            log.info("Event configuration found using tenantId as businessId fallback (Step 3: tenantId match): tenantId={}, eventType={}", tenantId, eventType);
            return Optional.of(result);
        }

        log.debug("Event configuration not found after all 3 fallback steps for businessId: {}, tenantId: {}, eventType: {}", businessId, tenantId, eventType);
