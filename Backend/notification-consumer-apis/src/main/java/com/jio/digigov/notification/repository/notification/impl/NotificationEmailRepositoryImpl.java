...
/**
 * Implementation of NotificationEmailRepository for email notification data access.
 *
 * Provides optimized MongoDB operations for email notifications with comprehensive
...
public class NotificationEmailRepositoryImpl implements NotificationEmailRepository {
    ...
    @Override
    public NotificationEmail save(NotificationEmail notification, MongoTemplate mongoTemplate) {
        try {
            ...
            log.error("Error saving email notification: {}", e.getMessage());
            throw new RuntimeException("Failed to save email notification", e);
        }
    }

    @Override
    public Optional<NotificationEmail> findByNotificationId(String notificationId, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("notificationId").is(notificationId));
            NotificationEmail notification = mongoTemplate.findOne(query, NotificationEmail.class);
            return Optional.ofNullable(notification);
        } catch (Exception e) {
            log.error("Error finding email notification by ID {}: {}", notificationId, e.getMessage());
            return Optional.empty();
        }
    }

    @Override
    public List<NotificationEmail> findByEventId(String eventId, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("eventId").is(eventId))
                    .with(Sort.by(Sort.Direction.DESC, "createdAt"));
            return mongoTemplate.find(query, NotificationEmail.class);
        } catch (Exception e) {
            log.error("Error finding email notifications by event ID {}: {}", eventId, e.getMessage());
            return List.of();
        }
    }

    @Override
    public List<NotificationEmail> findByCorrelationId(String correlationId, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("correlationId").is(correlationId))
                    .with(Sort.by(Sort.Direction.DESC, "createdAt"));
            return mongoTemplate.find(query, NotificationEmail.class);
        } catch (Exception e) {
            log.error("Error finding email notifications by correlation ID {}: {}", correlationId, e.getMessage());
            return List.of();
        }
    }

    @Override
    public List<NotificationEmail> findByStatus(String status, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("status").is(status))
                    .with(Sort.by(Sort.Direction.ASC, "createdAt"));
            return mongoTemplate.find(query, NotificationEmail.class);
        } catch (Exception e) {
            log.error("Error finding email notifications by status {}: {}", status, e.getMessage());
            return List.of();
        }
    }

    @Override
    public List<NotificationEmail> findByStatusWithLimit(String status, int limit, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("status").is(status))
                    .with(Sort.by(Sort.Direction.ASC, "createdAt"))
                    .limit(limit);
            ...
            log.error("Error finding email notifications by status {} with limit {}: {}",
    ...
    @Override
    public List<NotificationEmail> findRetryReady(LocalDateTime currentTime, MongoTemplate mongoTemplate) {
        try {
            ...
            log.error("Error finding retry-ready email notifications: {}", e.getMessage());
            return List.of();
        }
    }
    ...
    public long updateStatus(String notificationId, String status, MongoTemplate mongoTemplate) {
        try {
            ...
            return mongoTemplate.updateFirst(query, update, NotificationEmail.class).getModifiedCount();
        } catch (Exception e) {
            log.error("Error updating email notification status for ID {}: {}", notificationId, e.getMessage());
            return 0;
        }
    }
    ...
    public long updateStatusAndProcessedAt(String notificationId, String status,
        ...
        try {
            ...
            log.error("Error updating email notification status and processedAt for ID {}: {}",
    ...
    public long updateRetryInfo(String notificationId, int attemptCount, LocalDateTime nextRetryAt,
        ...
        try {
            ...
            log.error("Error updating email notification retry info for ID {}: {}",
                     notificationId, e.getMessage(), e);
            return 0;
        }
    }
    ...
    public long updateDeliveryInfo(String notificationId, String status, LocalDateTime sentAt,
        ...
        try {
            ...
            log.error("Error updating email notification delivery info for ID {}: {}",
                     notificationId, e.getMessage(), e);
            return 0;
        }
    }
    ...
    public long countByStatus(String status, MongoTemplate mongoTemplate) {
        try {
            Query query = new Query(Criteria.where("status").is(status));
            return mongoTemplate.count(query, NotificationEmail.class);
        } catch (Exception e) {
            log.error("Error counting email notifications by status {}: {}", status, e.getMessage());
            return 0;
        }
    }
    ...
    public long countByBusinessIdAndStatus(String businessId, String status, MongoTemplate mongoTemplate) {
        try {
            ...
            log.error("Error counting email notifications by business {} and status {}: {}",
    ...
    @Override
    public List<NotificationEmail> findPermanentlyFailed(MongoTemplate mongoTemplate) {
        try {
            Criteria criteria = new Criteria().andOperator(
            ...
            log.error("Error finding permanently failed email notifications: {}", e.getMessage());
            return List.of();
        }
    }
    ...
    public long deleteOldRecords(LocalDateTime cutoffDate, MongoTemplate mongoTemplate) {
        try {
            ...
            Query query = new Query(criteria);
            return mongoTemplate.remove(query, NotificationEmail.class).getDeletedCount();
        } catch (Exception e) {
            log.error("Error deleting old email notification records: {}", e.getMessage());
            return 0;
        }
    }
}