...
@Repository
public class SmscRepositoryImpl implements SmscRepository {

    private TenantMongoTemplateProvider mongoTemplateProvider;

    @Autowired
    public SmscRepositoryImpl(TenantMongoTemplateProvider mongoTemplateProvider) {
        this.mongoTemplateProvider = mongoTemplateProvider;
    }

    @Override
    public SmscConfig save(SmscConfig config) {
    ...
    @Override
    public SmscConfig findByConfigId(String configId) {
        MongoTemplate mongoTemplate = this.mongoTemplateProvider.getMongoTemplate(ThreadContext.get(Constants.TENANT_ID_HEADER));
        Criteria criteria = new Criteria();
        criteria.and("configId").is(configId);
        Query query = new Query();
        query.addCriteria(criteria);
        return mongoTemplate.findOne(query, SmscConfig.class);
    }

    @Override
    public List<SmscConfig> findConfigByParams(Map<String, String> searchParams) {
        MongoTemplate mongoTemplate = this.mongoTemplateProvider.getMongoTemplate(ThreadContext.get(Constants.TENANT_ID_HEADER));
        Criteria criteria = new Criteria();
        ...
        return mongoTemplate.find(query, SmscConfig.class);
    }
    ...
    public boolean existByScopeLevel(String scopeLevel) {
        ...
        return mongoTemplate.exists(query, SmscConfig.class);
    }
    ...
    public boolean existByScopeLevelAndBusinessId(String scopeLevel, String businessId) {
        ...
        return mongoTemplate.exists(new Query(criteria), SmscConfig.class);
    }
}
