...
public class GrievanceRepositoryImpl implements GrievanceRepository {
    ...
    @Autowired
    public GrievanceRepositoryImpl(TenantMongoTemplateProvider mongoTemplateProvider) {
        this.mongoTemplateProvider = mongoTemplateProvider;
    }

    @Override
    public GrievanceConfig save(GrievanceConfig config) {
    ...
    @Override
    public GrievanceConfig findByConfigId(String configId) {
        MongoTemplate mongoTemplate = this.mongoTemplateProvider.getMongoTemplate(ThreadContext.get(Constants.TENANT_ID_HEADER));
        Criteria criteria = new Criteria();
        criteria.and("configId").is(configId);
        Query query = new Query();
        query.addCriteria(criteria);
        return mongoTemplate.findOne(query, GrievanceConfig.class);
    }

    @Override
    public List<GrievanceConfig> findConfigByParams(Map<String, String> searchParams) {
        MongoTemplate mongoTemplate = this.mongoTemplateProvider.getMongoTemplate(ThreadContext.get(Constants.TENANT_ID_HEADER));
        Criteria criteria = new Criteria();
        ...
        return mongoTemplate.find(query, GrievanceConfig.class);
    }

    @Override
    public long count() {
        MongoTemplate mongoTemplate = this.mongoTemplateProvider.getMongoTemplate(ThreadContext.get(Constants.TENANT_ID_HEADER));
        return mongoTemplate.count(new Query(), GrievanceConfig.class);
    }
    ...
    public boolean existByScopeLevel(String scopeLevel) {
        ...
        return mongoTemplate.exists(query, GrievanceConfig.class);
    }
    ...
    public boolean existByScopeLevelAndBusinessId(String scopeLevel, String businessId) {
        ...
        return mongoTemplate.exists(new Query(criteria), GrievanceConfig.class);
    }
}
