...
## Overview
The `dpdp-audit-module` provides audit recording, PDF certificate generation (Section 65B style), and publishing audit events to Kafka for the DPDP Consent Management System. It is a Spring Boot microservice implemented in Java (Maven).
...
|---|---:|---|---|
| `SERVER_PORT` / `server.port` | `9006` | HTTP port | no |
| `SPRING_PROFILES_ACTIVE` / `spring.profiles.active` | `default` | Active profile | no |
| `MONGODB_URI` / `spring.data.mongodb.uri` | *(required)* | MongoDB connection URI | yes |
| `KAFKA_BOOTSTRAP_SERVERS` / `spring.kafka.bootstrap-servers` | *(required)* | Kafka brokers | no |
| `AUDIT_TOPIC` / `audit.kafka.topic` | `audit-events` | Kafka topic used for audit events | no |
...
curl -X POST http://localhost:9006/api/audits
...
- Docker (example):
    ...
    - `docker run -e MONGODB_URI="mongodb://host:27017/auditdb" -p 9006:9006 dpdp-audit-module:latest`
...
- Kafka errors: confirm `KAFKA_BOOTSTRAP_SERVERS`.
- MongoDB errors: verify `MONGODB_URI` and tenant database creation permissions.
