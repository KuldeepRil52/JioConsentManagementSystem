...
/**
 * Service that removes application logs older than configured retention days per tenant.
 * - Iterates active tenants (parallelized)
 * - Deletes old log documents in batches
 * - Sends per-tenant notification summary and writes stats
 */
@Service
@Slf4j
public class LogsRetentionService {
    ...
    @Value("${schedular.job.logs-retention.thread-pool-size:10}")
    private int threadPoolSize;

    @Value("${logs.default-retention-days:2}")
    private int defaultRetentionDays;

    @Autowired
    public LogsRetentionService(TenantRegistryRepository tenantRegistryRepository,
    ...
    public void init() {
        ...
        log.info("Initialized LogsRetentionService with thread pool size: {}", threadPoolSize);
    }

    @PreDestroy
    public void shutdown() {
        if (executorService != null) {
            log.info("Shutting down LogsRetentionService executor");
    ...
    public int processLogsRetention(int batchSize) {
        log.info("Starting LogsRetentionService - batchSize: {}", batchSize);
        ...
        if (activeTenants == null || activeTenants.isEmpty()) {
            log.warn("No active tenants found for LogsRetentionService");
            return 0;
        }

        List<Callable<TenantResult>> tasks = activeTenants.stream()
                .map(t -> (Callable<TenantResult>) () -> processTenantLogsRetention(t.getTenantId(), batchSize))
            ...
            for (int i = 0; i < futures.size(); i++) {
                ...
                try {
                    ...
                    if(result.resources().isEmpty()) {
                        log.info("No logs found for retention in tenant: {}", tenantId);
                        resources = List.of(createResource(Constants.TENANT_ID, tenantId, tenantId));
                    }
                    else {
                        log.info("Tenant: {} processed {} logs for retention", tenantId, processed);
                        resources = result.resources().stream()
                            .map(doc -> createResource("LOG_ID", doc.getString(Constants.LOG_ID), doc.getString("businessId")))  // change as per actual fields
                            .toList();
                    
                        logIds = result.resources().stream()
                            .map(doc -> doc.getString(Constants.LOG_ID)) //change as per actual fields
                            .collect(Collectors.toSet());
                    }

                    log.info("Saving LogsRetention stats for tenant: {} | processed={} | duration={}ms",
                            tenantId, processed, duration);

                    statsRepository.saveTenantSummary(tenantId, SchedularStats.builder()
                            .runId(runId.toString())
                            .jobName(JobName.LOGS_RETENTION_JOB)
                            .group(Group.LOGS)
                            .action(Action.LOGS_RETENTION)
                    ...
                    payload.put("processedIds", logIds);
                    ...
                    notificationManager.initiateRetentionNotification(
                            NotificationEvent.RETENTION_LOGS_EXPIRED,
                            tenantId,
                            customerIdentifiers,
                            payload,
                            LANGUAGE.ENGLISH,
                    ...
                    // Log audit event for logs retention
                    logLogsRetentionAudit(runId, logIds, ActionType.NOTIFICATION_SENT);
                    ...
                    statsRepository.saveTenantSummary(tenantId, SchedularStats.builder()
                            ...
                            .details(Map.of("batchSize", batchSize))
                            .build());
                    log.error("Tenant {} failed LogsRetention: {}", tenantId, e.getMessage(), e);
                    if (e instanceof InterruptedException) Thread.currentThread().interrupt();
                }
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.error("LogsRetentionService interrupted", e);
        }

        log.info("LogsRetentionService completed | runId={} | processed={}", runId, totalProcessed);
        return totalProcessed;
    }

    private TenantResult processTenantLogsRetention(String tenantId, int batchSize) {
        ThreadContext.put(Constants.TENANT_ID_HEADER, tenantId);
        log.info("Processing logs retention for tenant: {}", tenantId);
        int processed = 0;
        ...
        try {
            ...
            while (hasMore) {
                ...
                Criteria criteria = Criteria.where("timestamp").lte(cutoff);    //change field as per log document schema
                Query query = new Query(criteria).with(pageable);

                List<Document> docs = mongoTemplate.find(query, Document.class, "system_logs");
                docs = docs.stream()
            ...
            log.info("Tenant {} logs retention complete. Processed: {}", tenantId, processed);
            ...
            log.error("Error processing logs retention for tenant {}: {}", tenantId, e.getMessage(), e);
    ...
    private boolean notificationEventExists(Document doc, MongoTemplate mongoTemplate) {
        ...
        return mongoTemplate.exists(
                Query.query(
                    Criteria.where("event_type").is("RETENTION_LOGS_EXPIRED")
                            .and("event_payload.processedIds").is(doc.getString(Constants.LOG_ID)) //add conditions as/when required
                ),
    ...
    private RetentionWindow loadTenantRetentionWindow(MongoTemplate mongoTemplate) {
        try {
            Query q = new Query(Criteria.where("retentions.logs_retention").exists(true));
    ...
    /**
     * Modular function to log Logs retention audit events
     * Can be used in both create and update Logs retention flows
     *
     * @param logIds The Log Ids affected
     * @param actionType The action type (DELETE) //NOTIFICATION_SENT for now
     */
    public void logLogsRetentionAudit(UUID runId, Set<String> logIds, ActionType actionType) {
        try {
            ...
            extra.put(Constants.DATA, logIds);

            AuditRequest auditRequest = AuditRequest.builder()
                    .actor(actor)
                    .businessId(tenantId)
                    .group(String.valueOf(Group.LOGS))
                    .component(AuditComponent.LOGS_RETENTION)
