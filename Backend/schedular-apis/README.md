...
**Default port:** `9017` (overridable by `SERVER_PORT` environment variable)
...
```
Service will start on `http://localhost:9017/schedular` by default.
...
```powershell
docker run -e SERVER_PORT=9017 -e MONGODB_URI="mongodb://..." -p 9017:9017 schedular:local
```
...
- `SERVER_PORT` — `9017` — HTTP port the app listens on (mapped to Spring Boot `server.port`).
...
curl -H "tenantId: tenant-abc" \
  "http://localhost:9017/schedular/v1/stats"
```
