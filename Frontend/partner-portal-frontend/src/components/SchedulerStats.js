...
// import { Text, Spinner, ActionButton, InputFieldV2 } from '@jds/core';
// import { IcRefresh, IcClose, IcTime } from '@jds/core-icons';
...
import { useSelector } from 'react-redux';
import { Text, Spinner, ActionButton } from '../custom-components';
import { IcClose, IcTime, IcStatusSuccessful } from '../custom-components/Icon';
import config from '../utils/config';
import { isSandboxMode, sandboxAPICall } from '../utils/sandboxMode';
import '../styles/schedulerStats.css';
import '../styles/templates.css';
...
const SchedulerStats = () => {
  ...
  // Start date can only be within 15 days before end date
  const getMinStartDate = (selectedEndDate) => {
  ...
  useEffect(() => {
    ...
    const minStartDate = getMinStartDate(endDate);
  ...
  // Fetch scheduler stats
  const fetchSchedulerStats = useCallback(async (isRefresh = false, signal) => {
    if (isRefresh) {
      setRefreshing(true);
    } else {
      setLoading(true);
  ...
  }, [tenantId, businessId, token, startDate, endDate]);

  useEffect(() => {
    if (!tenantId || !businessId) return;
    const controller = new AbortController();
    fetchSchedulerStats(false, controller.signal);
    ...
    <div className="configurePage">
      ...
      <div className="template-table-controls" style={{ justifyContent: 'flex-start', marginBottom: '1.5rem' }}>
        ...
        >
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (minStartDate && selectedStartDate < minStartDate) {
                  setStartDate(minStartDate);
          ...
          <ActionButton kind="primary" size="medium" label="Apply Filter" onClick={handleDateFilterApply} />
        </div>
      </div>
      ...
      {(error ||
        !schedulerData ||
        !schedulerData.data ||
        (Array.isArray(schedulerData.data) && schedulerData.data.length === 0)) && (
          <div style={{ textAlign: 'center', padding: '40px', marginRight: '2rem' }}>
            <Text appearance="body-m" color="primary-grey-80">
              No data available
            </Text>
                ...
                <tbody>
                  {(() => {
                    ...
                    schedulerData.data.forEach((item) => {
                      ...
                      if (
                        item.timestamp &&
                        (!aggregatedData[jobName].lastExecutionTime ||
                          new Date(item.timestamp) > new Date(aggregatedData[jobName].lastExecutionTime))
                      ) {
                        aggregatedData[jobName].lastExecutionTime = item.timestamp;
                      }
                    ...
                    return schedulerList.map((scheduler, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            <span
                              className={`status-badge ${rate >= 90 ? 'status-published' : rate >= 70 ? 'status-draft' : 'status-inactive'
                                }`}
                            >
                              {rate}%
                            </span>
