...
const EditCategoryModal = ({ onClose, category, onUpdate }) => {
  const [activeLanguage, setActiveLanguage] = useState(languages[0].value);
  const [startIndex, setStartIndex] = useState(0);
  const [cookieName, setCookieName] = useState(category.category);
  ...
  }, [activeLanguage, cookieName, description]);
  ...
  const handleUpdate = () => {
    if (cookieName && description) {
      onUpdate(cookieName, description);
    }
  };
  ...
  return (
    <div className="modal-outer-container">
      <div className="master-set-up-modal-container">
        ...
        <Text appearance="heading-xs">Edit cookie category</Text>
        ...
        <div className="modal-add-btn-container">
          <ActionButton label="Update" kind="primary" onClick={handleUpdate} />
        </div>
      </div>
    </div>
  );
