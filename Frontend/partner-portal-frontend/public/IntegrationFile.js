...
var darkTheme = "";
var lightTheme = "";
var globalFontFamily = "system-ui";
var globalFontSize = "14px";
var globalFontWeight = "400";
var globalFontStyle = "normal";

async function loadDashboardTheme() {
  try {
    const url = `${process.env.REACT_APP_CONSENT_URL}/v1.0/user-dashboard-theme/get`;
        ...
        if (settings) {
          ...
          if (settings.fontFile) {
            ...
            if (!document.getElementById(styleId)) {
              ...
              style.textContent = `
                @font-face {
                  font-family: 'JioCustomFont';
                  src: url(${fontSrc});
                }
              `;
              document.head.appendChild(style);
            }
            globalFontFamily = "'JioCustomFont', system-ui, sans-serif";
          }
        }
      }
    }
...
async function fetchDocument(doc_id) {
  try {
    const url = `${process.env.REACT_APP_CONSENT_URL}/v1.0/documents/view-document/${doc_id}`;
...
async function callBhashini(selectedLangKey) {
  try {
    ...
    const url = `${process.env.REACT_APP_TRANSLATOR_URL}/v1/translate`;
    const options = [
          ...
          if (showCheckbox) {
            ...
            if (consentArray[preferenceId]) checkbox.checked = true;
                    ...
                    input.addEventListener("change", (e) => {
                      ...
                      if (detailName.toLowerCase().includes('email')) {
                      ...
                      if (detailName.toLowerCase().includes('mobile')) {
                      ...
                      if (isValid) {
                        e.target.style.borderColor = '#ccc'; // Reset border
                        dynamicInputValues[detailName] = value;
                      } else {
                        showToast(errorMessage, 'error');
                        e.target.style.borderColor = 'red'; // Visual feedback
                        dynamicInputValues[detailName] = ''; // Clear invalid value
                      }
                    });
...
async function captureConsent(consentArray, isParental = false) {
  if (
    ...
    try {
      if (parentalFlow && !isParental) {
        openUnder18Modal(consentArray);
        return;
      }

      showDotLoader("Capturing Your Consent");
      const url = `${process.env.REACT_APP_CONSENT_URL}/v1.0/consent/create`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          isParentalConsent: isParental || parentalFlow,
        }),
      });
...
async function captureAllConsent(consentArray, isParental = false) {
  try {
    if (parentalFlow && !isParental) {
      openUnder18Modal(consentArray);
      return;
...
async function fetchConsentData() {
  try {
    const url = `${process.env.REACT_APP_CONSENT_URL}/v1.0/consent-handle/get/${consentHandleId}`;
    const response = await fetch(url, {
      method: "GET",
      headers: {
        txn: "7afd223d-56c7-4556-9b2c-3032a684bcbd",
    ...
    cusId = data?.customerIdentifiers?.value;
    ...
    data?.preferences?.forEach((pref) => {
      ...
      const obj = {
        ...
        Duration: `${pref.preferenceValidity.value} ${pref.preferenceValidity.unit.charAt(0).toUpperCase() +
          pref.preferenceValidity.unit.slice(1).toLowerCase()
          }`,
...
async function callBhashiniConfig(tenant, business) {
  try {
    const url = `${process.env.REACT_APP_TRANSLATOR_URL}/v1/translateConfig`;
...
const loadPreferenceCenter = async (handleId, tenant, business) => {
  consentHandleId = handleId;
  tenantId = tenant;
  businessId = business;
  showLoader();
  // Fetch dashboard theme/typography first
  await loadDashboardTheme();
  // Call Bhashini configuration API
...
`;
  ...
  textA.setAttribute("font-family", globalFontFamily);
  ...
  textHindi.setAttribute("font-family", globalFontFamily);
  ...
  function openUnder18Modal(consentArray) {
    ...
    stepText.style.fontFamily = globalFontFamily;
    ...
    function createStyledDropdown(defaultOption = "Select") {
      ...
      const options = [defaultOption, "Digilocker", "Declaration Based"];
      ...
      dropdown.addEventListener("change", () => {
        ...
        // note: initial renderInitial sets up event to toggle fields, but keep this in case
      });
    ...
    function renderOTPStep() {
      ...
      verifyBtn.onclick = () => {
        ...
        parentalFlow = false;
        if (consentArray) {
          captureConsent(consentArray, true);
        }
  ...
  labelSpan.style.fontWeight = globalFontWeight;
  labelSpan.style.fontSize = "16px";
  labelSpan.style.fontFamily = globalFontFamily;

  const valueSpan = document.createElement("span");
  valueSpan.innerText = `: ${cusId}`;
  valueSpan.style.fontWeight = "400";
  valueSpan.style.fontSize = "16px";
  valueSpan.style.fontFamily = globalFontFamily;
  ...
  label.style.fontSize = globalFontSize;
  label.style.fontFamily = globalFontFamily;
  ...
  label2.style.fontFamily = globalFontFamily,
  ...
  checkboxLabel.style.fontSize = globalFontSize;
  // checkboxLabel.style.color = "rgba(0, 0, 0, 0.65)";
  checkboxLabel.style.fontWeight = "bold";
  checkboxLabel.style.fontFamily = globalFontFamily;
  ...
  ConsentAllow.style.fontSize = globalFontSize;
  // ConsentAllow.style.color = cardFontColor;
  ConsentAllow.style.fontWeight = "500";
  ConsentAllow.style.fontFamily = globalFontFamily;
