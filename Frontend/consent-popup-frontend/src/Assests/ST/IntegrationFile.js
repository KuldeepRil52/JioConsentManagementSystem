...
var overlay = "";
var DigilockerData = "";
const KYC_DIGILOCKER = "DIGILOCKER";
const KYC_SMS = "SMS_LINK";
var select = "";
var option = "";
...
function startDigilockerFlow(consentArray) {
  showLoader();

  const responseType = "code";
  const codeChallengeMethod = "S256";

  generateDigilockerTransactionId(consentArray).then(async (consentMetaId) => {
    if (!consentMetaId) {
      hideLoader();
      return;
    }
    ...
    const codeChallenge = await generateCodeChallenge(digilockercodeVerifier);
...
function openUnder18Modal(consentArray) {
  ...
  function createStyledDropdown(
    ...
    if (hasDigilockerData)
      options.push({
        label: "Proceed with KYC of the parent/legal guardian using DigiLocker",
        value: KYC_DIGILOCKER,
      });
    options.push({
      label:
        "Send the consent link to the parent/legal guardian over SMS/email and obtain approval",
      value: KYC_SMS,
    });
...
function openParentalRelationModal(consentArray) {
  // Overlay
  const modalOverlay = document.createElement("div");
  Object.assign(modalOverlay.style, {
    position: "fixed",
  ...
  Object.assign(modalBox.style, {
    ...
    width: "420px",
    padding: "20px",
    background: cardBackgroundColor || "#fff",
    color: cardFontColor || "#000",
    position: "relative",
    fontFamily: "system-ui",
  });
  ...
  closeBtn.onclick = closeParentalModal;

  /* ---------------- Header Title ---------------- */
  const headerTitle = document.createElement("div");
  headerTitle.innerText =
    window.relationModalTranslation?.relation ||
  ...
  const createRadio = (translationKey, fallbackText, value) => {
    const wrapper = document.createElement("div");
    wrapper.style.marginBottom = "12px";

    const radio = document.createElement("input");
    radio.type = "radio";
    radio.name = "guardianType";
    radio.value = value;
    ...
    label.innerText =
      window.relationModalTranslation?.[translationKey] || fallbackText;

    if (!window.relationModalElements) {
      window.relationModalElements = {};
    }
    window.relationModalElements[translationKey] = label;
  ...
  const parentOpt = createRadio("parent", "Parent", "PARENT");
  const guardianOpt = createRadio(
    "legalGuardian",
    "Legal Guardian",
    "LEGAL_GUARDIAN",
  );
  const otherOpt = createRadio("other", "Other", "OTHER");

  /* ---------------- Other Textbox ---------------- */
  const otherInputWrapper = document.createElement("div");
  Object.assign(otherInputWrapper.style, {
    marginLeft: "24px",
    marginTop: "8px",
    display: "none",
  });
  ...
  otherInput.placeholder = "Please specify relationship";
  ...
  otherInputWrapper.appendChild(otherInput);

  /* ---------------- Radio Behaviour ---------------- */
  otherOpt.radio.addEventListener("change", () => {
    otherInputWrapper.style.display = "block";
  });

  parentOpt.radio.addEventListener("change", () => {
    otherInputWrapper.style.display = "none";
    otherInput.value = "";
  });

  guardianOpt.radio.addEventListener("change", () => {
    otherInputWrapper.style.display = "none";
    otherInput.value = "";
  });

  optionsContainer.appendChild(parentOpt.wrapper);
  optionsContainer.appendChild(guardianOpt.wrapper);
  optionsContainer.appendChild(otherOpt.wrapper);
  optionsContainer.appendChild(otherInputWrapper);
  ...
  clearBtn.innerText = window.relationModalTranslation?.clear || "Clear";
  ...
  clearBtn.onclick = () => {
    parentOpt.radio.checked = true;
    guardianOpt.radio.checked = false;
    otherOpt.radio.checked = false;
    otherInputWrapper.style.display = "none";
    otherInput.value = "";
  };

  /* -------- Save Button -------- */
  const relSaveBtn = document.createElement("button");
  relSaveBtn.innerText = window.relationModalTranslation?.save || "Save";
  Object.assign(relSaveBtn.style, {
    padding: "8px 20px",
    borderRadius: "24px",
    background: btn2BackgroundColor || "#007bff",
    color: btn2FontColor || "#fff",
    fontSize: "14px",
    fontWeight: "600",
    border: "none",
    cursor: "pointer",
  ...
  relSaveBtn.onclick = () => {
    const selected = document.querySelector(
      'input[name="guardianType"]:checked',
    );

    if (!selected) {
      showToast("Please select a relationship", "error", 3000);
      return;
    }

    if (selected.value === "OTHER" && !otherInput.value.trim()) {
      showToast("Please specify the relationship", "error", 3000);
      return;
    }

    window.selectedGuardianType = selected.value;
    window.otherGuardianText = otherInput.value || "";

    modalOverlay.remove();
    startDigilockerFlow(consentArray);
  };

  footer.appendChild(clearBtn);
  footer.appendChild(relSaveBtn);
  ...
  modalBox.appendChild(headerTitle);
...
async function sendParentalConsentRequest(parentName, parentIdentity) {
  try {
    ...
    const url =
      "https://api.jcms-st.jiolabs.com:8443/negd/consent/v1.0/consent-handle/parental-consent";
...
async function fetchDocument(doc_id) {
  try {
    const url = `https://api.jcms-st.jiolabs.com:8443/negd/consent/v1.0/documents/view-document/${doc_id}`;
...
async function callBhashini(selectedLangKey) {
  try {
    ...
    const url =
      "https://api.jcms-st.jiolabs.com:8443/negd/translator/translate";
    const options = [
    ...
    const staticInputs = [
      ...
      { id: "relation", source: "Select your relationship with the child" },
      { id: "parent", source: "Parent" },
      { id: "legalGuardian", source: "Legal Guardian" },
      { id: "other", source: "Other" },
      { id: "pleaseSpecify", source: "Please specify relationship" },
      { id: "clear", source: "Clear" },
      { id: "save", source: "Save" },
    ...
    window.relationModalTranslation = {
      relation: getTarget("relation"),
      parent: getTarget("parent"),
      legalGuardian: getTarget("legalGuardian"),
      other: getTarget("other"),
      pleaseSpecify: getTarget("pleaseSpecify"),
      clear: getTarget("clear"),
      save: getTarget("save"),
    };
...
function applyRelationModalTranslations() {
  if (!window.relationModalElements) return;

  Object.keys(window.relationModalElements).forEach((key) => {
    const el = window.relationModalElements[key];
    const translatedText = window.relationModalTranslation?.[key];
...
async function generateDigilockerTransactionId(consentArray) {
  ...
  try {
    ...
    let relationshipValue = "parent";

    if (window.selectedGuardianType === "PARENT") {
      relationshipValue = "Parent";
    } else if (window.selectedGuardianType === "LEGAL_GUARDIAN") {
      relationshipValue = "Legal Guardian";
    } else if (window.selectedGuardianType === "OTHER") {
      relationshipValue = window.otherGuardianText || "";
    }
    const url =
      "https://api.jcms-st.jiolabs.com:8443/negd/consent/v1.0/consent-meta/create";
    ...
    try {
      if (parentalFlow == true) {
        ...
        if (isParental) {
          // showLoader();
          //overlay.remove();
          openParentalRelationModal(consentArray);
        ...
        showDotLoader("Capturing Your Consent");
        const url =
          "https://api.jcms-st.jiolabs.com:8443/negd/consent/v1.0/consent/create";
...
async function fetchConsentData() {
  try {
    const url = `https://api.jcms-st.jiolabs.com:8443/negd/consent/v1.0/consent-handle/parental-consent/${consentHandleId}`;
    const response = await fetch(url, {
      method: "GET",
      headers: {
        txn: "7afd223d-56c7-4556-9b2c-3032a684bcbd",
...
async function fetchDigilockerData() {
  try {
    const url = `https://api.jcms-st.jiolabs.com:8443/negd/partnerportal/v1.0/digilocker/credential/search`;
    const response = await fetch(url, {
      method: "GET",
      headers: {
        txn: generateUUID(),
...
function changeDesc(selectedLangKey) {
  callBhashini(selectedLangKey).then(() => {
    applyRelationModalTranslations();
  });
}
...
async function callBhashiniConfig(tenant, business) {
  try {
    const url =
      "https://api.jcms-st.jiolabs.com:8443/negd/translator/translateConfig";
...
const loadPreferenceCenter = async (
  ...
  console.log("Consent Data recieved:", consentData);
...
`;
  ...
  mainBody.appendChild(TopDesc);
  mainBody.appendChild(customerId);
  mainBody.appendChild(accordion);
  mainBody.appendChild(accordion2);
  mainBody.appendChild(documentLink);
  if (parentalControl == true) {
    mainBody.appendChild(checkboxContainer);
  }

  mainBody.appendChild(withdrawContainer);
  mainBody.appendChild(ConsentAllow);
